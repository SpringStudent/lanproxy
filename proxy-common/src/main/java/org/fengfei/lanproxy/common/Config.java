...
public class Config {
    ...
    public String getProjectSn() {
        ...
        try {
            //项目打包成jar包所在的根路径
            String rootPath = System.getProperty("app.dir");
            if (rootPath == null || rootPath.equals("")) {
                ApplicationHome applicationHome = new ApplicationHome(Config.class);
                rootPath = applicationHome.getSource().getParentFile().toString();
            }
            String configFilePath = rootPath + "/start.bat";
            File file = new File(configFilePath);
            InputStreamReader read = new InputStreamReader(new FileInputStream(file), "UTF-8");//考虑到编码格式
            BufferedReader bufferedReader = new BufferedReader(read);
            String lineTxt = null;
            while ((lineTxt = bufferedReader.readLine()) != null) {
                int idx = lineTxt.indexOf("-Diot.projectSn=");
                if (idx != -1) {
                    projectSn = lineTxt.substring(idx).substring(0, lineTxt.substring(idx).indexOf(" ")).split("=")[1];
                    break;
                }
            }
            read.close();
