...
public class ProxyClientContainer implements Container, ChannelStatusListener {
    ...
    private void connectProxyServer() {
        String serverHost = System.getProperty("server.host", config.getStringValue("server.host"));
        Integer serverPort = LangUtil.parseInt(System.getProperty("server.port", config.getStringValue("server.port")));
        //代理客户端连接到代理服务器
        bootstrap.connect(serverHost, serverPort).addListener(new ChannelFutureListener() {
            ...
            public void operationComplete(ChannelFuture future) throws Exception {
                if (future.isSuccess()) {
                    ...
                    //projectSn
//                    proxyMessage.setUri(config.getProjectSn());
//                    proxyMessage.setUri("123456");
