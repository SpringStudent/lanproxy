...
import org.fengfei.lanproxy.server.config.web.dao.UserDao;
    ...
    public static void init() {
        ...
        ApiRoute.addMiddleware(new RequestMiddleware() {
            ...
            public void preRequest(FullHttpRequest request) {
                ...
                if (!authenticated && auth != null) {
                    ...
                    if (authArr.length == 2) {
                        User user = UserDao.getUser(authArr[0]);
                        if (user != null && authArr[0].equals(user.getUsername()) && authArr[1].equals(user.getPassword())) {
                            authenticated = true;
                        }
                    }
                }
        ...
        ApiRoute.addRoute("/config/update", new RequestHandler() {
            ...
            public ResponseInfo request(FullHttpRequest request) {
                ...
                try {
                    ProxyConfig.getInstance().update(clients, false);
                } catch (Exception ex) {
